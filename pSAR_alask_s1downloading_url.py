...
#
#       python ./download-all-2018-05-10_14-56-18.py
#
#    Download all files in a Metalink/CSV: (downloaded from ASF Vertex)
#
#       python ./download-all-2018-05-10_14-56-18.py /path/to/downloads.metalink localmetalink.metalink localcsv.csv
#
...
if len(sys.argv)<2:
   helpstr = \
        '''
        ++++++++++++++++++++++++++++++
        %s [list] [wget as downloader] 
           -usr [tigilfeng in default] -pwd[]
           -printonly
        
        +
        -usr  keyword for user name, tigilfeng/wasar
              two available user accounts

        by Wanpeng Feng, @SYSU, Guangzhou, 2019/04/05
...
#
# A function from Wanpeng Feng, @CCRS/NRCan, 2018-05-10
#
def read_s1list(in_list):
    datalist = []
    counter = 0
...
#
#
global in_list, s1db,s1dbs
#
s1db = '/media/wafeng/S1Backup/s1data'
#
# switch S1DB2 as default backup folder
#
s1db0= os.environ['S1DB']
s1db = os.environ['S1DB']
s1db = '/mnt/NAS80T/S1/'
s1dbs = ['/mnt/NAS80T_2/S1/','/mnt/NAS80T_3/S1/','/mnt/NAS80T/S1/']
#
in_list = 's1.list'
#
if len(sys.argv) > 1:
...
try:
   ...
   from urllib2 import URLError, HTTPSHandler,  HTTPHandler, HTTPRedirectHandler, HTTPCookieProcessor
...
#
   ...
   from urllib.request import HTTPHandler, HTTPSHandler, HTTPRedirectHandler, HTTPCookieProcessor
...
def backupzip(s1zip):
    # s1db = '/media/wafeng/S1Backup/s1data'
    #
    # convert a symbolic path to its absolute path...
    # added by Wanpeng Feng, @SYSU, Zhuhai, 2021/01/07
    s1zip = os.path.realpath(s1zip)
    s1zip = os.path.basename(s1zip)
    #
    for s1db in s1dbs:
      targetZIP  = os.path.join(s1db, s1zip)
      #
      #
      #
    ...
    #
    #
    # return the fullpath name of s1zip
    #
    s1zip_fullpath = os.path.normpath(os.path.dirname(os.path.abspath(s1zip)))
    s1dbpath = os.path.normpath(s1db)
    s1dbpath0 = os.path.normpath(s1db0)
    #
    #
    # to make sure the current working folder is not $S1DB
    #
    if s1zip_fullpath == s1dbpath:
        goflag = False
    else:
        goflag = True
    #
    ...
    #
       ...
       #
       if not os.path.islink(s1zip):
          #
          if os.path.exists(targetZIP):
              os.remove(targetZIP)
          #
          # print(s1zip,targetZIP)
          shutil.copyfile(s1zip,targetZIP)
          #
          if zipfile.is_zipfile(targetZIP) and not os.path.islink(targetZIP):
             os.remove(s1zip)
             os.symlink(targetZIP,s1zip)
          # 
    else:
       #
       if os.path.exists(targetZIP) and zipfile.is_zipfile(targetZIP):
          #
          if os.path.exists(s1zip):
             os.remove(s1zip)
          #
          if not os.path.exists(s1zip):
             os.symlink(targetZIP,s1zip)
         
    return True
    #
def checkifdone(s1zip):
    #
    # s1db = '/media/wafeng/S1Backup/s1data'
    # targetZIP = s1db+'/'+os.path.basename(s1zip)
    # targetZIP0 = s1db0+'/'+os.path.basename(s1zip)
    #
    for s1db in s1dbs:
        #
        targetZIP = s1db+'/'+os.path.basename(s1zip)
        ...
        if zipfile.is_zipfile(targetZIP):
           if not os.path.exists(os.path.basename(s1zip)):
              #
              if os.path.exists(targetZIP):
                 os.system('ln -s %s %s' % (targetZIP,os.path.basename(s1zip)))
              else:
                 os.system('ln -s %s %s' % (targetZIP0,os.path.basename(s1zip)))
           return targetZIP,True
    #
    return 'NULL', False
#
def download_aria2(in_list):
    #
    datafiles = read_s1list(in_list);
    #
    for cfile in datafiles:
        #
        aria2_GO = 'aria2c -c --http-auth-challenge=false --http-user=%s --http-passwd=%s "%s"' % \
                    (usr,pwd,cfile)
        bname = os.path.basename(cfile).split('.zip')[0]
        #
        # /home/wafeng/Programs/MY_SCR/pSAR_alask_s1downloading_url.py
        aria2_GO = 'aria2c -c --http-auth-challenge=true --http-user=%s --http-passwd=%s "%s" --check-certificate=false -o %s' % (usr, pwd, cfile, cfile.split('/')[-1])
        # https://d18me4youk6849.cloudfront.net/SLC/
        # aria2_GO = 'aria2c -c --http-auth-challenge=true --http-user=%s --http-passwd=%s --input-file="https://d18me4youk6849.cloudfront.net/SLC/%s.zip" --check-certificate=false -o %s.zip' % (usr,pwd,bname,bname)
        #
        myzipfile,flag = checkifdone(cfile.split('/')[-1])
        print(cfile)
        #
        if not flag and not printonly:
           print("%s" % aria2_GO)
        #
        #
        if not flag and not printonly:
...
class bulk_downloader:
    def __init__(self):
        ...
        # the file list will be imported from a local s1.list 
        #
        if os.path.exists(in_list):
           datafiles = read_s1list(in_list) 
        else:
           print(" Error: please provide data list file (%s) in the current folder..." % in_list)
           sys.exit(-1)
        #
        print(datafiles)
        #         
                ...
                if arg == '--insecure':
                    ...
                    if os.path.isfile( arg ):
                        ...
                        if arg.endswith('.metalink'):
                            new_files = downloader.process_metalink(arg)
                        else:
                            new_files = downloader.process_csv(arg)
    ...
    def download_files(self):
        for file_name in self.files:
            ...
            ####
            # 
            # check if the file has been stored in the s1db
            # 
            myzipfile,flag = checkifdone(os.path.basename(file_name))
            if flag:
              if os.path.exists(myzipfile) and not os.path.exists(os.path.basename(myzipfile)):
                 os.symlink(myzipfile,os.path.basename(myzipfile))
            # set a timer
            start = time.time()
...
if __name__ == "__main__":
    ...
    '''    
     +++++++++++++++++++++++++++++++++++++
     %s <in_list> [downloader]
     +++++++++++++++++++++++++++++++++++++
     
    '''
    print(helpstr % os.path.basename(sys.argv[0]))
    #
    if dbtool.upper() == 'PYTHON':
       downloader = bulk_downloader()
       downloader.download_files()
       downloader.print_summary()
    elif dbtool.upper() == 'WGET':
       download_wget(in_list);
    else:
       download_aria2(in_list);
    #
    # update EOF(s) 
    # updated by FWP, @SYSU, Guangzhou, 2020/02/04
    if len(glob.glob('S*.zip'))<1:
        print(" Warning: no S1*.zip was found in %s" % os.getcwd())
        sys.exit(-1)
    #
